...
void	sprite_shoot(t_env *e, int mov, int i)
{
	...
	{
		e->slist.s[i].state += (((int)e->p.bobbing % 2) == 1 ? e->delay : 0);
...
int		path_finding(t_env *e, int i)
{
	...
	if ((e->slist.s[i].texture == 34 && e->slist.s[i].dist > 32) || (e->slist.s[i].texture == 44 && e->slist.s[i].dist > 150))
	{
		if (y < x)
		{
			x = (e->slist.s[i].x > e->p.x ? -3 : 3);
			if (e->map.floor[(int)e->slist.s[i].y / BLOC][(int)(e->slist.s[i].x + x) / BLOC] == e->map.floor[(int)e->slist.s[i].y / BLOC][(int)e->slist.s[i].x / BLOC])
			{
				e->slist.s[i].x += x;
				return (0);
			}
		...
		else
		{
			...
			if (e->map.floor[(int)(e->slist.s[i].y + y)/ BLOC][(int)e->slist.s[i].x / BLOC] == e->map.floor[(int)e->slist.s[i].y / BLOC][(int)e->slist.s[i].x / BLOC])
			{
				e->slist.s[i].y += y;
				return (0);
			}
		}
		e->slist.s[i].state = (e->slist.s[i].state == 0 ? (int)e->p.bobbing % 2 : 0);
	}
	return (1);
}

void	get_damage(t_env *e)
{
...
void	sprite_handling(t_env *e)
{
	...
	while (++i < e->slist.n)
	{
		if (e->slist.s[i].life < 0)
		{
			...
			e->slist.s[i].state = 0;
		if (e->slist.s[i].attitude == 1 && e->slist.s[i].dist < 200 && e->slist.s[i].life > 0)
		{
